import csv
from effects.buffs import Buff, Debuff
...
def read_character_data():
    with open(CHARACTER_FILENAME) as charfile:
        ...
        for row in reader:
            ...
            newname.normal_attack = [float(item) for item in row['normal_attack'].split(',')
                                     if row['normal_attack'].split(',') != [""]]
...
def read_ele_ratio_data():
    with open(ELERATIO_FILENAME) as ele_ratio_file:
        ...
        for row in reader:
            ...
            ratiodict[level] = pctstr_to_float(row['Ratio'])
    return ratiodict
